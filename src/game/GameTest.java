...
public class GameTest {
	...
	private static boolean checkEq(Object a, Object b, String message) {
		return check(a.equals(b), String.format("%s: Expected \"%s\", but got \"%s\"", message, b, a));
	}
	...
	private static boolean checkSuper(Class<?> sub, Class<?> sup) {
		return check(sub.getSuperclass().equals(sup),
				sub.getName() + " should be a direct subclass of " + sup.getName());
	}
...
// Testing games 
	...
	private static void testBoard() {
		new AccessChecker(Board.class).tF("n").tF("m").tF("board").pM("get").pM("toString").pM("isEmpty").pM("isFull")
				.tM("set").tM("maxLineContaining").checkAll();
	...
	private static void testGame() {
		new AccessChecker(Game.class).tF("players").tF("s").tM("onePlay").tM("doesWin").pM("play").checkAll();
